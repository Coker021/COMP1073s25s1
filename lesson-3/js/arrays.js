...
let myArray = ["string", 100, false, [5, "hello"]];
...
console.log(`The 2nd element of myArray is ${String(myArray[3][1])}.`)
...
output.textContent = `myArray hss ${myArray.length} elements`;
output.textContent = "";
...
for (let i = 0; i < myArray.length; i++){
    if(Array.isArray(myArray[i])){
        ...
        for (j= 0; j < myArray[i].length; j++){
            output.textContent += "This is an array"
        }
    }
    else{
...
let orig6 = "Chelsea, Barcelona, Real Madrid, Juventus, Bayern Munich";
...
orig6Length = orig6Array.unshift("Brighton","Alaves");
