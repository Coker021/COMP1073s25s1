...
btn.addEventListener('click', function() {
	...
	for (let i = 0; i < cities.length; i++){
		...
		if(searchName.toLowerCase() === cities[i].toLowerCase()){
			/* STEP 2c: If the searchName matches a city, set the result textContent to that city and exit the loop with 'break' */
			result.textContent = `Found: ${cities[i]}`;
			break;
		}
		else{
			/* STEP 2d: If the searchName does not match a city, set the result textContent to 'Not found' */
...
while (j < cities.length) {
	if (searchName.toLowerCase() === cities[j].toLowerCase()) {
		outputWhile = `Found: ${cities[j]}`;
		break;
	} else {
		outputWhile = `Sorry, ${searchName} Not found.`;
	}
	j++;
}
